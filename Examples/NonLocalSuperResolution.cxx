...
int
NonLocalSuperResolution(itk::ants::CommandLineParser * parser)
{
...
InitializeCommandLineOptions(itk::ants::CommandLineParser * parser)
{
  ...
  {
    std::string description = std::string("A low-resolution image input image to be superresoluted.  ");
  ...
  {
    std::string description = std::string("Search radius.  Default = 3x3x3.");
  ...
  {
    std::string description = std::string("Intensity difference sigma.  Default = 1.0");
  ...
  {
    std::string description = std::string("Patch similarity sigma.  Default = 1.0");
  ...
  {
    std::string description = std::string("Scale levels.  Default = 32x16x8x2x1");
...
// 'main()'
int
NonLocalSuperResolution(std::vector<std::string> args, std::ostream * /*out_stream = nullptr */)
{
  ...
  std::string commandDescription =
    ...
    std::string("Non-local MRI Upsampling.") + std::string("Medical Image Analysis, 14:784-792, 2010 and") +
  ...
  else
  {
    ...
    itk::ants::CommandLineParser::OptionType::Pointer interpolatedImageOption = parser->GetOption("interpolated-image");
    itk::ants::CommandLineParser::OptionType::Pointer referenceImageOption = parser->GetOption("reference-image");
